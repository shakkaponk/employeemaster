...
//     data = xStmt._rows; //it has data in it in format [0, "k", "a", "k.a"] 
 //  var	columnNames = xStmt._columnInfo; //it has column Names
	...
	// // //Insert Record into DB Table and Temp Output Table
		var pStmt;
			pStmt = param.connection.prepareStatement("insert into \"employee.employee\" values(?,?,?,?,?,?,?)" );			
	var testVar = "K";
		pStmt.setInteger(1, 1232);
		pStmt.setString(2,testVar );		
		pStmt.setString(3, testVar);
		pStmt.setString(4, testVar);
		pStmt.setString(5, testVar);
		pStmt.setString(6, "tDict.JOBTITLE");
		pStmt.setString(7, "tDict.DEPARTMENT");
		pStmt.executeUpdate();
		pStmt.close();
